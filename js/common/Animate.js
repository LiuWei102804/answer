    ...
    if( AnimatePlatform ) {
        var Animate = {
                        ...
                        for( var attr in json ) {
                            ...
                            switch( attr ) {
                                ...
                                case "skew" :
                                    ...
                                    var r = isNaN( fx( t , curr["rotate"][0] , json["rotate"][0] - curr["rotate"][0] , times ) ) ? 0 :fx( t , curr["rotate"][0] , json["rotate"][0] - curr["rotate"][0] , times ) ;
                                    var cosVal = Math.cos( r * Math.PI / 180 ),
                                        sinVal = Math.sin( r * Math.PI / 180 ) ,
                                        a = isNaN( fx( t , curr["scale"][0] , json["scale"][0] - curr["scale"][0] , times ) - 1 ) ? 0 : fx( t , curr["scale"][0] , json["scale"][0] - curr["scale"][0] , times ) - 1;
                                        b = isNaN( fx( t , curr["skew"][0] , json["skew"][0] - curr["skew"][0] , times ) ) ? 0 : fx( t , curr["skew"][0] , json["skew"][0] - curr["skew"][0] , times ) ,
                                        c = isNaN( fx( t , curr["skew"][1] , json["skew"][1] - curr["skew"][1] , times ) ) ? 0 : fx( t , curr["skew"][1] , json["skew"][1] - curr["skew"][1] , times ) ,
                                        d = isNaN( fx( t , curr["scale"][1] , json["scale"][1] - curr["scale"][1] , times ) - 1 ) ? 0 : fx( t , curr["scale"][1] , json["scale"][1] - curr["scale"][1] , times ) - 1 ,
                                        e = isNaN( fx( t , curr["translate"][0] , json["translate"][0] - curr["translate"][0] , times ) ) ? 0 :fx( t , curr["translate"][0] , json["translate"][0] - curr["translate"][0] , times ) ,
                                        f = isNaN( fx( t , curr["translate"][1] , json["translate"][1] - curr["translate"][1] , times ) ) ? 0 : fx( t , curr["translate"][1] , json["translate"][1] - curr["translate"][1] , times );
            ...
            getTransform : function ( obj ) {
                var val = this.getStyle( obj , "transform") != "none" ? this.getStyle( obj , "transform") : "matrix(1, 0, 0, 1, 0, 0)";
                //console.log( val )
            ...
            setDefaultTransform : function ( obj , attr , val ) {
                ...
                if( val[attr] &&  val[attr].length > 0 ) {
                    if( !val[attr][1] && attr !== "rotate" ) {
                        ...
                        val[attr].splice( 1 ,transform[attr][1] );
                    }
                } else {
